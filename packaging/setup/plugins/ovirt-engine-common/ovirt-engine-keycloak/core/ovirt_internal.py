...
class Plugin(plugin.PluginBase):
    ...
    def _setup_ovirt(self):
        ...
        if password:
            ...
            self.logger.info(_('Creating realm in with Keycloak CLI admin'))
            self._setup_realm()

            # please note that client_id is an id (hash) and not the human readable
            #  'client id' defined as okkcons.Const.KEYCLOAK_INTERNAL_CLIENT_NAME
            self.logger.info(_('Creating internal client in with Keycloak CLI admin'))
            ...
            administrator_group_id = self._setup_ovirt_administrator_user_group()
            self._setup_internal_admin_user(
                administrator_group_id=administrator_group_id,
                password = password,
            )
            self.logger.info(_('Done with setting up Keycloak for Ovirt Engine'))
        ...
        if realm_id is None:
            ...
            self.execute(
                (
                    okkcons.FileLocations.KEYCLOAK_CLI_ADMIN_SCRIPT,
                    'update',
                    'realms/{}'.format(okkcons.Const.KEYCLOAK_INTERNAL_REALM),
                    '-s', 'enabled=true',
                )
            )
        return realm_id
    ...
    def _setup_client(self):
        ...
        if cid is None:
            rc, stdout, stderr = self.execute(
                (
                    ...
                    '-s', 'enabled=true',
                    '-s', 'clientId={}'.format(okkcons.Const.KEYCLOAK_INTERNAL_CLIENT_NAME),
                    '-s', 'clientAuthenticatorType=client-secret',
                    '-s', 'secret={}'.format(
                        self.environment[okkcons.ConfigEnv.KEYCLOAK_OVIRT_INTERNAL_CLIENT_SECRET],
                    ),
                    '-i',
                ),
            )
        ...
        # Once basic client id exists then let's update it with desired configuration
        engine_host = self.environment[
            oenginecons.ConfigEnv.ENGINE_FQDN
        ]
        self.execute(
    ...
    def _setup_client_scope(self, scope_name, client_id):
        ...
        self.execute(
            (
                ...
                'update',
                '-r', okkcons.Const.KEYCLOAK_INTERNAL_REALM,
                "clients/{client_id}/optional-client-scopes/{client_scope_id}".format(
                    client_id = client_id,
    ...
    def _setup_ovirt_administrator_user_group(self):
        group_id = self._get_group_id(okkcons.Const.OVIRT_ADMINISTRATOR_USER_GROUP_NAME)
        if group_id is None:
            rc, stdout, stderr = self.execute(
                (
                    okkcons.FileLocations.KEYCLOAK_CLI_ADMIN_SCRIPT,
                    ...
                    'groups',
                    '-r', okkcons.Const.KEYCLOAK_INTERNAL_REALM,
                    '-s', 'name={}'.format(okkcons.Const.OVIRT_ADMINISTRATOR_USER_GROUP_NAME),
                    '-i',
                )
            )
    ...
    def _setup_internal_admin_user(self, password, administrator_group_id):
        ...
        admin_user_id = self._get_user_id(admin_user)
    ...
    def _get_user_id(self, username):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'username', username)
    ...
    def _get_client_id(self, client_id_name):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'clientId', client_id_name)
    ...
    def _get_realm_id(self, realm_name):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'realm', realm_name)
    ...
    def _get_client_scope_id(self, scope_name):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'name', scope_name)
    ...
    def _get_protocol_mapper_id(self, protocol_mapper_name, client_id):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'name', protocol_mapper_name)
    ...
    def _get_group_id(self, group_name):
        ...
        return self._get_id_from_response(self._results(rc, stdout), 'name', group_name)
    ...
    def _results(rc, out):
        if rc == 1:
            raise RuntimeError("Error configuring internal Keycloak instance. Check setup log for details")
